...
viem_colors = {
    "dark": {"text_bg": "black", "text_fg":"lime", "cursor":"brown", "selectbg":"Green"}, 
    "light": {"text_bg": "white", "text_fg":"lime", "cursor":"black", "selectbg":"Green"}
}
...
fontss = {
    ...
    "Minecraftia":{
        "font": ("Minecraftia", 14, "bold")
    }
}

win.title("Editing a text file")
win.iconbitmap("sourse/nico.ico")
win.geometry("800x700")

def changetheme(theme):
    entr["bg"] = viem_colors[theme]["text_bg"]
    entr["fg"] = viem_colors[theme]["text_fg"]
    entr["insertbackground"] = viem_colors[theme]["cursor"]
    entr["selectbackground"] = viem_colors[theme]["selectbg"]

def changefont(fonts):
    entr["font"] = fontss[fonts]["font"]

def noexit():
    answer = ms.askokcancel("Hardening", "Are you sure you want to get out?")
    if answer:
        sys.exit(0)

def save_file():
    f = io.open(namefile, "w", encoding="UTF-8")
    textforsave = entr.get("1.0", END)
    f.write(textforsave)
...
win2.iconbitmap("sourse/nico.ico")
win2.withdraw()
...
plugpath = pathname[:-5]
plugpath = plugpath + "/plugins for text edit"
pathname = pathname[:-5]
...
file_menu.add_command(label="Close", command=noexit)
win.config(menu=file_menu)

#view
view_menu = Menu(win, tearoff=0)
view_menu_sub = Menu(view_menu, tearoff=0)
font_menu = Menu(view_menu, tearoff=0)
font_menu_sub = Menu(font_menu, tearoff=0)
view_menu_sub.add_command(label="Dark", command=lambda:changetheme("dark"))
view_menu_sub.add_command(label="light", command=lambda:changetheme("light"))
view_menu.add_cascade(label="Theme", menu=view_menu_sub)

font_menu_sub.add_command(label="Arial", command=lambda:changefont("Arial"))
font_menu_sub.add_command(label="Comic Sans Ms", command=lambda:changefont("Comic Sans MS"))
font_menu_sub.add_command(label="Times New Roman", command=lambda:changefont("Times New Roman"))
font_menu_sub.add_command(label="Minecraftia", command=lambda:changefont("Minecraftia"))

view_menu.add_cascade(label="Font", menu=font_menu_sub)

mmenu.add_cascade(label="File", menu=file_menu)
mmenu.add_cascade(label="View", menu=view_menu)
...
entr.delete("1.0", END)
entr.insert("1.0", open(namefile, encoding="UTF-8").read())
entr.pack(fill="both")
